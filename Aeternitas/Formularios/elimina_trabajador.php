...
<head>
	...
	<title>Elimina Trabajador</title>
    <script>
  function confirmarEliminacion() {
    var confirmacion = prompt("¿Estás seguro de que quieres eliminar a este empleado? Introduce la palabra 'ELIMINAR' para confirmar.");
...
<body>
    <div class="row">
        ...
        <FORM METHOD="POST" action="elimina_trabajador.php">	
		...
		<input type="submit" name="action" value="Mostrar" class="btn btn-primary">
            ...
            <form id="msform" name="fvalida" method="POST" action="../PHP_Aeternitas/eliminar_trabajador.php">
                <!-- progressbar -->
                <ul id="progressbar">
				...
				<?php
					...
					echo "
						<fieldset>
							...
							<br><br>
							<label> ID del Trabajador a Eliminar </label>
							...
							<input type='text' name='area' value='" . $row['area'] . "' readonly/>
                            <label>Empleado supervisor</label>
                            <input type='number' name='at_epo_id' value='" . $row['at_epo_id'] . "' readonly/>
